...
namespace CodeWars
{
    public static class Sudoku
    {
        public static bool ValidateSolution(int[][] board)
        {
            ...
            static bool DontPassChecks (int[] arr, int sumToCheck) 
                => arr.Sum () != sumToCheck || arr.Any (c => c == 0) || arr.Distinct ().Count () != 9;

            // Check 9x9 rows and columns
            for (var i = 0; i < board.Length; i++) {
                var row = board[i];
                var column = Enumerable.Range (0, board.GetLength (0)).Select (x => board[x][i]).ToArray ();
                
                if (DontPassChecks(row, sum) ||DontPassChecks(column, sum)) {
                    sum = 0;
                    break;
                }
